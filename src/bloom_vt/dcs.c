...
static void format_intro(BvtTerm *vt, uint8_t final) {
    ...
    for (int i = 0; i < p->param_count && pos < sizeof(buf) - 8; ++i) {
        ...
        if (n < 0) break;
        pos += (size_t)n;
        if (pos >= sizeof(buf)) { pos = sizeof(buf) - 1; break; }
    }
    for (int i = 0; i < p->intermediate_count && pos < sizeof(buf) - 2; ++i)
        buf[pos++] = (char)p->intermediates[i];
    if (pos < sizeof(buf) - 1) buf[pos++] = (char)final;
    buf[pos] = '\0';

    size_t to_copy = pos;
    if (to_copy > sizeof(p->dcs_intro) - 1) to_copy = sizeof(p->dcs_intro) - 1;
