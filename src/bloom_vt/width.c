...
int bvt_codepoint_width(BvtTerm *vt, uint32_t cp) {
    ...
    if (cp < 0xA0u) return 0;            /* DEL + C1 */
    if (cp == 0x00ADu) return 0;         /* soft hyphen */

    if (range_lookup(ZERO, ARRAY_LEN(ZERO), cp)) return 0;
    if (range_lookup(WIDE, ARRAY_LEN(WIDE), cp)) return 2;
...
static bool is_extend(uint32_t cp) {
    ...
    if (cp == 0x200Du) return true;                      /* ZWJ */
    if (cp >= 0x0300u && cp <= 0x036Fu) return true;     /* combining */
    if (cp >= 0xFE00u && cp <= 0xFE0Fu) return true;     /* variation selectors */
    if (cp >= 0xFE20u && cp <= 0xFE2Fu) return true;
    if (cp >= 0xE0100u && cp <= 0xE01EFu) return true;   /* VS supp */
    if (cp >= 0xE0020u && cp <= 0xE007Fu) return true;   /* tag chars */
    if (cp >= 0x1F3FBu && cp <= 0x1F3FFu) return true;   /* skin tones */
...
static bool has_vs16(const uint32_t *cps, uint32_t len) {
    for (uint32_t i = 0; i < len; ++i)
        if (cps[i] == 0xFE0Fu) return true;
    return false;
}

static bool has_vs15(const uint32_t *cps, uint32_t len) {
    for (uint32_t i = 0; i < len; ++i)
        if (cps[i] == 0xFE0Eu) return true;
    return false;
}
...
int bvt_cluster_width(BvtTerm *vt, const uint32_t *cps, uint32_t len) {
    ...
    if (has_vs16(cps, len)) return 2;
    /* VS15 forces text presentation → 1 cell. */
    if (has_vs15(cps, len)) return 1;
...
bool bvt_grapheme_break_before(uint32_t prev, uint32_t cur, void *state_in) {
    ...
    (void)state_in;
    if (prev == 0u) return false; /* SOT — first codepoint of cluster */
    /* GB3: CR × LF */
    if (prev == 0x0Du && cur == 0x0Au) return false;
    /* GB4-5: control × any, any × control → break. Controls hit GROUND
     * before reaching the cluster accumulator, so this is mostly
     * defensive. */
    if (cur == 0x0Au || cur == 0x0Du || cur == 0x00) return true;
    /* GB9 / GB9a: × Extend, × ZWJ, × SpacingMark → no break. */
    if (is_extend(cur)) return false;
