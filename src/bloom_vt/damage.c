...
static void union_rect(BvtRect *acc, BvtRect r) {
    if (r.start_row < acc->start_row) acc->start_row = r.start_row;
    if (r.start_col < acc->start_col) acc->start_col = r.start_col;
    if (r.end_row   > acc->end_row)   acc->end_row   = r.end_row;
    if (r.end_col   > acc->end_col)   acc->end_col   = r.end_col;
}
...
void bvt_damage_row(BvtTerm *vt, int row) {
    ...
    if (!vt->damage_dirty) { vt->damage = r; vt->damage_dirty = true; }
    else                   { union_rect(&vt->damage, r); }
}
