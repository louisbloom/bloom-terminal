...
static void damage_union(BvtBackendData *d, int t, int l, int b, int r) {
    if (!d->needs_redraw) {
        d->damage_top = t; d->damage_bottom = b;
        d->damage_left = l; d->damage_right = r;
        d->needs_redraw = true;
    } else {
        if (t < d->damage_top) d->damage_top = t;
        if (b > d->damage_bottom) d->damage_bottom = b;
        if (l < d->damage_left) d->damage_left = l;
        if (r > d->damage_right) d->damage_right = r;
    }
}
...
static void cb_set_mode(BvtMode mode, bool on, void *user) {
    BvtBackendData *d = user;
    switch (mode) {
        case BVT_MODE_CURSOR_VISIBLE: d->cursor_visible = on; break;
        case BVT_MODE_CURSOR_BLINK:   d->cursor_blink   = on; break;
        case BVT_MODE_ALTSCREEN:      d->altscreen      = on; break;
        case BVT_MODE_MOUSE_X10:       d->mouse_mode = on ? 1 : 0; break;
        case BVT_MODE_MOUSE_BTN_EVENT: d->mouse_mode = on ? 1 : 0; break;
        case BVT_MODE_MOUSE_DRAG:      d->mouse_mode = on ? 2 : 0; break;
        case BVT_MODE_MOUSE_ANY_EVENT: d->mouse_mode = on ? 3 : 0; break;
        default: break;
    }
}
...
static void cb_sb_push(const BvtCell *c, int n, bool w, void *u) {
    (void)c; (void)n; (void)w; (void)u;
}
static void cb_sb_pop(BvtCell *o, int n, void *u) {
    (void)o; (void)n; (void)u;
}
...
static void convert_cell(BvtTerm *vt, const BvtCell *src, TerminalCell *dst) {
    ...
    if (st->attrs & BVT_ATTR_DHL_TOP)         dst->attrs.dhl = 1;
    else if (st->attrs & BVT_ATTR_DHL_BOTTOM) dst->attrs.dhl = 2;
...
static bool bvt_back_get_damage_rect(TerminalBackend *term,
    ...
    BvtBackendData *d = term->backend_data;
    if (!d || !d->needs_redraw || !rect) return false;
...
static void bvt_back_send_mouse_event(TerminalBackend *term, int row, int col,
    ...
    if (!d) return;
    BvtMods mods = 0;
    if (mod & TERM_MOD_SHIFT) mods |= BVT_MOD_SHIFT;
    if (mod & TERM_MOD_ALT)   mods |= BVT_MOD_ALT;
    if (mod & TERM_MOD_CTRL)  mods |= BVT_MOD_CTRL;
    BvtMouseButton b = BVT_MOUSE_NONE;
    switch (button) {
        case 1: b = BVT_MOUSE_LEFT;       break;
        case 2: b = BVT_MOUSE_MIDDLE;     break;
        case 3: b = BVT_MOUSE_RIGHT;      break;
        case 4: b = BVT_MOUSE_WHEEL_UP;   break;
        case 5: b = BVT_MOUSE_WHEEL_DOWN; break;
        default: break;
    }
...
static BvtKey map_key(int key) {
    switch (key) {
        case TERM_KEY_ENTER:     return BVT_KEY_ENTER;
        case TERM_KEY_TAB:       return BVT_KEY_TAB;
        case TERM_KEY_BACKSPACE: return BVT_KEY_BACKSPACE;
        case TERM_KEY_ESCAPE:    return BVT_KEY_ESCAPE;
        case TERM_KEY_UP:        return BVT_KEY_UP;
        case TERM_KEY_DOWN:      return BVT_KEY_DOWN;
        case TERM_KEY_LEFT:      return BVT_KEY_LEFT;
        case TERM_KEY_RIGHT:     return BVT_KEY_RIGHT;
        case TERM_KEY_INS:       return BVT_KEY_INS;
        case TERM_KEY_DEL:       return BVT_KEY_DEL;
        case TERM_KEY_HOME:      return BVT_KEY_HOME;
        case TERM_KEY_END:       return BVT_KEY_END;
        case TERM_KEY_PAGEUP:    return BVT_KEY_PAGEUP;
        case TERM_KEY_PAGEDOWN:  return BVT_KEY_PAGEDOWN;
        case TERM_KEY_F1:        return BVT_KEY_F1;
        case TERM_KEY_F2:        return BVT_KEY_F2;
        case TERM_KEY_F3:        return BVT_KEY_F3;
        case TERM_KEY_F4:        return BVT_KEY_F4;
        case TERM_KEY_F5:        return BVT_KEY_F5;
        case TERM_KEY_F6:        return BVT_KEY_F6;
        case TERM_KEY_F7:        return BVT_KEY_F7;
        case TERM_KEY_F8:        return BVT_KEY_F8;
        case TERM_KEY_F9:        return BVT_KEY_F9;
        case TERM_KEY_F10:       return BVT_KEY_F10;
        case TERM_KEY_F11:       return BVT_KEY_F11;
        case TERM_KEY_F12:       return BVT_KEY_F12;
        default:                  return BVT_KEY_NONE;
    }
}
